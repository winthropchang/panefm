...
- PaneFM 的核心價值是操作檔案與目錄，因此所有功能都必須以「快、準、穩定」為
  不可退讓的驗收條件，不能用新增功能作為犧牲基本操作速度或正確性的理由。
- 開啟大型目錄、切換目錄、複製、移動、刪除與重新整理不得在 TUI 主執行緒執行
  可預期會長時間阻塞的遞迴掃描或大量 I/O；畫面應先進入目標狀態，再增量補齊資料。
- 任何修改上述流程的變更，都必須加入大型列表、背景工作或失敗清理的回歸測試；
  macOS 與 Windows 共用邏輯不得只驗證其中一個平台的正常路徑。
- 檔案與目錄容量在 macOS、Windows 一律使用 1024 進位的 `B/K/M/G/T` 顯示，確保
  同一份資料在兩個平台的 PaneFM 畫面中得到一致結果。
...
- macOS 與 Windows 的本機單檔 copy 使用 `std::fs::copy`，資料夾傳輸固定使用三條
  file worker；不可退化成單 worker 逐檔等待，也不可用過多 worker 壓垮 SMB server。
- 資料夾 copy 必須一邊廣度走訪、一邊把單檔工作排入有界佇列；task 的總 byte 數由
  走訪器逐步回報，不可為了進度顯示同步或平行重掃整棵來源樹。
- 小於 8 MiB 的檔案不可為了進度輪詢再建立額外 thread；大量小檔案的效能主要取決於
  減少 thread 建立、重複 metadata 與不必要的同步等待。
- PaneFM 的 Trash、task history 等執行期資料不可預設存入目前工作目錄；macOS 與
  Windows 應使用各自的使用者資料目錄，避免複製專案時把應用程式狀態一起遞迴複製。
- 檔案操作不可根據 `.tfm` 或其他特殊名稱擅自排除內容；使用者要求複製目錄時，
  副本必須完整包含所有可讀取項目。執行期狀態應改存使用者資料區，不可靠 copy 過濾補救。
- SMB 若明確回報平台原生 copy 不支援，必須先清除原生 API 留下的 partial 目標，再以
  有界 buffer 串流重試；權限、容量不足與連線錯誤不可被 fallback 隱藏。
- 原生 copy 回傳量必須與走訪時取得的來源大小一致；串流 fallback 另需在關閉 handle 後
  確認目標大小。失敗必須清理 partial，清理也失敗時要把兩個錯誤一起顯示。
...
- 大型資料夾的容量掃描不可阻擋 copy 啟動；若 byte 進度需要總量，掃描必須和 copy 並行，
  避免先完整走訪一次、再從頭複製一次所造成的可感知延遲。
- `linemode size` 的目錄容量必須在 panel 專屬背景 worker 中遞迴計算；掃描中約每
  200ms 批次更新目前 byte，切換目錄或模式時必須取消舊工作，禁止在 TUI thread
  同步展開目錄樹。
- 目錄容量掃描必須使用有上限的平行工作池：直接子目錄多時同時計算多列，
  只有單一大目錄時把可用額度用於該子樹；不可回到依列表順序逐個完整掃描。
- 列表大小顯示使用十進位 `B/KB/MB/GB/TB`，數值代表一般檔案的 logical bytes，不是
  檔案系統的 allocation size。掃描中的數值要加 `~`，完成後才移除，避免誤當精確終值。
...
- 每一個修過的 bug 都必須補測試
- 不能只修目前看到的問題而不建立保護
- **所有的 test 必須和核心程式碼完全拆開，嚴禁將測試 code 寫在核心業務邏輯檔案中**

### 9.0 測試程式碼分離原則（強制規定）

- **嚴禁寫在一起**：任何新增或修改的功能，絕對不可在核心實作檔案底部內嵌 `#[cfg(test)] mod tests { ... }`。
- **專屬 `tests/` 目錄**：所有測試程式碼一律放置在專屬的測試目錄中（專案根目錄的 `tests/` 目錄），維持核心業務邏輯的乾淨、專注與可維護性。
- **測試檔案命名**：各功能模組的測試檔案應獨立建立，命名採用 `tests/<module>_test.rs`（例如 `tests/diff_test.rs`、`tests/undo_backup_test.rs`）。
- **模組介面設計**：被測試引用的功能與資料結構應維持乾淨的 `pub` 介面，讓 `tests/` 能夠像外部呼叫者一樣進行清晰、嚴謹的功能與回歸測試。
...
- 要補哪些測試？
- **測試代碼是否規劃放在專屬的 `tests/` 目錄中？（嚴禁將測試寫在核心程式碼檔案內！）**
