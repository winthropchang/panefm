...
```
 ...
 ├───────────────┤  ├───────────────┤    ├───────────────┤  ├───────────────┤
 │• 自由分割佈局 │  │• h/j/k/l 移動 │    │• 視覺多選 (v) │  │• fd 檔名搜尋  │
 │• 1~9 視窗秒切 │  │• 數字 Count   │    │• CoW 零秒複製 │  │• rg 內容檢索  │
 │• 原生終端同步 │  │• fzf / zoxide │    │• 交易式 Undo  │  │• Tab 語法預覽 │
 └───────┬───────┘  └───────┬───────┘    └───────┬───────┘  └───────┬───────┘
         │                  │                    │                  │
 ┌───────┴───────┐  ┌───────┴───────┐    ┌───────┴───────┐  ┌───────┴───────┐
 │ 目錄差異矩陣  │  │ 智慧書籤歷史  │    │ 主題外觀視圖  │  │ 任務與說明    │
 ├───────────────┤  ├───────────────┤    ├───────────────┤  ├───────────────┤
 │• 2~N 目錄對齊 │  │• 單鍵秒跳書籤 │    │• 10+ 款主題   │  │• 背景 Worker  │
 │• 4 種差異過濾 │  │• 檔案自動監聽 │    │• 動態即時預覽 │  │• ? 當前面板   │
 │• 呼叫外部比對 │  │• 網路磁碟防卡 │    │• 5 種欄位模式 │  │• 統一 q 退出  │
 └───────────────┘  └───────────────┘    └───────────────┘  └───────────────┘
```
