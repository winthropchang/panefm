...
* **同欄/同列智慧均等分割**：同方向分割時（如連續 `wl` 或 `wj`），系統自動將整欄或整列視窗重新均等劃分（2 個各 50%、3 個各 33%），計算餘數自動分配給最後建立的視窗，無縫杜絕切割縫隙；巢狀分割時亦完美維持外層欄寬，於內部獨立均分高度。
...
* **互鎖邊界防護**：內建最小安全寬度（10 欄）與高度（3 列）極限保護，防止自己或鄰居被擠壓破圖。
...
| | `1` ~ `9` | 直接切換至指定編號的視窗 |
| | `w` | 開啟視窗管理選單（`wh/wj/wk/wl` 分割、`wr` 調整、`w=` 均等、`wc` 關閉、`wo` 獨佔、`wt` 開新終端） |
| | `wr` | 進入連續尺寸調整模式（`h/l` 增減寬度、`j/k` 增減高度、`=` 均等、`Esc/Enter` 退出） |
| | `w=` | 一鍵將所有分割視窗重新均等平分（Equalize） |
