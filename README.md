...
* **EasyMotion 全螢幕標籤直達（`e` / `:easymotion`）**：按下 `e` 進入跳轉模式，畫面所有可見檔案前方覆蓋高對比亮黃字母標籤（如 `[j]`, `[k]`, `[d]`, `[f]`, `[a]`...），按下標籤字母游標瞬間瞬移到該列，2 鍵直達螢幕上任何檔案！
...
* **`Tab` 萬能預覽系統（文字語法高亮、ANSI 圖片、目錄偷窺與壓縮包內部檢視）**：
  ...
  * **鄰近檔案背景預熱（Adjacent Speculative Prefetching）**：游標在清單停駐時，背景執行緒自動預加載並完成當前檔案與前後相鄰檔案（`selected ± 1`）的語法高亮並存入多項目 LRU 快取池（容量 32 筆），不僅清單模式隨手按 `Tab` 100% 0ms 秒開，在預覽中更可按 `[` / `]` 連續切換上/下一檔案，達到真正的 0.0ms 極速無感切換！
...
| | `gg` / `G` | 跳至頂部 / 跳至底部 |
| | `e` / `:easymotion` | **EasyMotion 瞬移**：全螢幕高亮字母標籤，單鍵瞬移直達 |
...
| **預覽與輔助**| `Tab` | 開啟/關閉全 Pane 預覽（支援清單模式背景預熱，0ms 秒開） |
| | `[` / `]` | 預覽模式中連續切換上一個 / 下一個檔案（支援數字前綴如 `3]`） |
