...
```mermaid
flowchart TD
    Core["<b>⚡ PaneFM 核心系統架構</b>"]

    subgraph G1["🪟 視窗與極速導航"]
        M1["<b>🪟 多視窗佈局</b><br>• 自由水平/垂直分割 (1~9 視窗)<br>• 原生終端 OSC 7 分頁目錄同步"]
        M2["<b>⚡ 極速導航與跳轉</b><br>• Vim 鍵位 (h/j/k/l & 數字 Count)<br>• fzf 模糊 / zoxide 歷史 / List Find"]
    end

    subgraph G2["📦 檔案與全域搜尋"]
        M3["<b>📦 安全檔案操作</b><br>• 視覺連續多選 (v) / 安全貼上<br>• CoW 零空間瞬間複製 / 交易式 Undo"]
        M4["<b>🔍 全域搜尋與預覽</b><br>• fd 檔名串流 / rg 內容全文檢索<br>• Tab 檔案語法高亮預覽與搜尋"]
    end

    subgraph G3["⚖️ 比對與智慧管理"]
        M5["<b>⚖️ 目錄差異比對矩陣</b><br>• 2~N 目錄同時對齊比對<br>• 4 種狀態過濾 / 整合外部比對器"]
        M6["<b>🔖 智慧書籤與歷史</b><br>• 單鍵快捷秒跳 / 檔案系統自動監聽<br>• SMB 網路磁碟防卡死安全取消"]
    end

    subgraph G4["🎨 外觀與智慧輔助"]
        M7["<b>🎨 豐富主題與視圖</b><br>• 10+ 款精美主題 (動態即時預覽)<br>• 5 種欄位模式 / 靈活排序"]
        M8["<b>📊 背景任務與說明</b><br>• 非同步 Worker 傳輸不卡 UI<br>• ? 當前面板情境速查 / 統一 q 退出"]
    end
...
| 核心分類 | 系統模組 | 核心特色能力 |
|:---|:---|:---|
| **視窗與導航** | 🪟 **多視窗佈局** | 自由水平/垂直分割、1~9 視窗秒切、獨立目錄狀態、原生終端 OSC 7 分頁目錄同步 |
| | ⚡ **極速導航跳轉** | `h/j/k/l` & 數字 Count、`g` 目錄跳轉、`fzf` 樹狀跳轉、`zoxide` 常用目錄、`/` List Find |
| **檔案與搜尋** | 📦 **安全檔案操作** | 視覺連續多選 (`v`)、CoW 零空間瞬間複製、Regex 批次改名、交易式 Undo (`u`)、垃圾桶一鍵還原 |
| | 🔍 **全域搜尋預覽** | `s` 檔名串流搜尋 (`fd`)、`S` 全文內容檢索 (`rg`)、`Tab` 檔案語法高亮預覽與關鍵字搜尋 |
| **比對與管理** | ⚖️ **差異比對矩陣** | 2~N 多目錄同時對齊、4 種狀態過濾、`.gitignore` 規則即時切換、呼叫外部比對器 (`delta`) |
| | 🔖 **智慧書籤歷史** | 單鍵快捷書籤秒跳、檔案系統即時監聽無感刷新、SMB 網路路徑支援與防卡死安全取消 |
| **外觀與輔助** | 🎨 **主題與視圖** | 10+ 款高質感主題（即時動態預覽）、5 種欄位顯示模式、多維度排序、隱藏檔切換 |
| | 📊 **任務與說明** | 背景 Worker 傳輸不卡 UI、`?` 當前面板情境感知速查、`F1` 完整字典、統一 `q`/`Esc` 退出 |
