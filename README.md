...
```
         ...
         │              │                │              │
 ┌───────┴──────────┐ ┌─┴────────────────┐ ┌────────────┴─┐ ┌──────────────────┐
 │   多視窗佈局管理 │ │   極速導航與跳轉 │ │   安全檔案操作復原 │ │   全域即時搜尋預覽 │
 ├──────────────────┤ ├──────────────────┤ ├──────────────────┤ ├──────────────────┤
 │ - 自由水平垂直分割 │ │ - 經典按鍵移動跳轉 │ │ - 視覺連續多選標記 │ │ - 檔名全域串流搜尋 │
 │ - 一至九號視窗秒切 │ │ - 支援數字計數前綴 │ │ - 零空間秒速檔案複製 │ │ - 檔案內容全文檢索 │
 │ - 原生終端分頁同步 │ │ - 模糊與歷史目錄跳 │ │ - 交易式復原與垃圾桶 │ │ - 檔案語法預覽搜尋 │
 └───────┬──────────┘ └─┬────────────────┘ └────────────┬─┘ └──────────────────┘
         │              │                │              │
 ┌───────┴──────────┐ ┌─┴────────────────┐ ┌────────────┴─┐ ┌──────────────────┐
 │   目錄差異比對矩陣 │ │   智慧書籤歷史管理 │ │   主題外觀自訂視圖 │ │   背景任務智慧說明 │
 ├──────────────────┤ ├──────────────────┤ ├──────────────────┤ ├──────────────────┤
 │ - 多目錄同時對齊比 │ │ - 單鍵快捷秒跳書籤 │ │ - 十款精美主題切換 │ │ - 非同步背景傳輸檔 │
 │ - 四種狀態差異過濾 │ │ - 檔案系統自動監聽 │ │ - 動態即時預覽效果 │ │ - 當前面板情境速查 │
 │ - 呼叫外部比對工具 │ │ - 網路磁碟防卡安全 │ │ - 多種欄位顯示模式 │ │ - 統一快速離開按鍵 │
 └──────────────────┘ └──────────────────┘ └──────────────────┘ └──────────────────┘
```
