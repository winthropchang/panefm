...
├── 🪟 多視窗佈局
│   ├── 自由水平/垂直分割（1~9 視窗秒切）
│   └── 原生終端 OSC 7 分頁目錄自動廣播同步
├── ⚡ 極速 Vim 導航跳轉
│   ├── h/j/k/l 移動、gg/G 與數字 Count 前綴
│   └── fzf 模糊跳轉、zoxide 常用目錄、/ List Find
├── 📦 安全檔案操作與復原
│   ├── 視覺連續多選 (v)、CoW 零空間瞬間複製
│   └── Regex 批次改名、交易式 Undo (u)、垃圾桶一鍵還原
├── 🔍 全域即時搜尋與預覽
│   ├── fd 檔名串流即時搜尋 (s)
│   ├── rg 內容全文檢索 (S) 與即時比對
│   └── Tab 檔案語法高亮預覽與關鍵字搜尋
├── ⚖️ N-Way 目錄差異比對矩陣
│   ├── 2~N 多目錄同時對齊比對 (Alt+d / :diff)
│   └── 4 種差異過濾、.gitignore 切換、呼叫外部比對器
├── 🔖 智慧書籤與歷史管理
│   ├── 單鍵快捷秒跳 (ba / b{key})
│   └── 檔案系統即時監聽、SMB 網路防卡死安全取消
├── 🎨 豐富主題與自訂視圖
│   ├── 10+ 款精美主題（動態即時預覽切換）
│   └── 5 種欄位顯示模式 (m)、靈活排序 (,)、隱藏檔切換
└── 📊 背景任務與智慧說明
    ├── 背景非同步 Worker 傳輸不卡 UI (T)
    ├── ? 當前面板情境感知速查、F1 全局完整字典
    └── 統一 Esc / q 快速退出與取消
```
