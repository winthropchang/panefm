...
| **Phase 13** | `src/file_manager/pane/preview.rs`<br>**(1,017 行)** | 拆分為 `pane/preview/` 目錄下 **6 個專用子模組**，門面僅 **99 行** | `97db494` | ✅ 單檔最大僅 341 行，預覽渲染、標題格式化、滾動邊界、搜尋跳轉、背景預熱與狀態控制徹底解耦 |

---

## 2. 目前剩餘大型檔案規模統計 (> 1,000 行)

截至 Phase 13 完成後，程式碼庫中仍超過 1,000 行的檔案如下（已依優先級排序）：

```text
優先級   檔案路徑                              目前行數    主要職責
─────────────────────────────────────────────────────────────────────────────
[P1]    src/file_manager/app/mod.rs          1,643 行   App 結構體定義、初始化、全域生命週期與事件調度
[P2]    src/file_manager/app/tests.rs       10,835 行   整合測試套件（可按領域拆分為多個測試檔案）
```
...
### 推薦第一順位：`src/file_manager/app/mod.rs` (1,643 行)
**目標**：抽取 App 生命週期與初始化。
