...
截至 Phase 9 完成後，程式碼庫中仍超過 1,000 行的檔案如下（已依優先級排序）：

```text
優先級   檔案路徑                              目前行數    主要職責
─────────────────────────────────────────────────────────────────────────────
[P1]    src/file_manager/app/mod.rs          1,643 行   App 結構體定義、初始化、全域生命週期與事件調度
[P2]    src/config.rs                        1,362 行   TOML 設定檔載入、驗證、持久化、預設範本與快捷鍵配置
[P3]    src/file_manager/app/status.rs       1,258 行   底部狀態列動態快捷鍵提示、狀態字串折疊與格式化
[P4]    src/file_manager/app/keys/mod.rs     1,100 行   Normal 模式按鍵調度、計數前綴 (Count)、貼上事件
[P5]    src/file_manager/pane/preview.rs     1,017 行   PaneState 專屬預覽視窗滾動、搜尋跳轉、非同步預熱
[P6]    src/file_manager/app/tests.rs       10,835 行   整合測試套件（可按領域拆分為多個測試檔案）
```
...
### 推薦第一順位：`src/config.rs` (1,362 行)
**目標**：拆分設定檔讀寫與資料結構。
- **`config/schema.rs`** (~450 行)：`AppConfig`, `GeneralConfig`, `UiConfig`, `NavigationConfig` 等資料結構定義與預設值。
- **`config/keys.rs`** (~350 行)：自訂鍵盤映射結構體與按鍵配置解析。
- **`config/persist.rs`** (~300 行)：TOML 序列化、範本寫入、主題持久化 (`persist_theme`)。
- **`config/mod.rs`** (~200 行)：門面與 `load_config` 流程。
