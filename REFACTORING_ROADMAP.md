...
| **Phase 15** | `src/file_manager/platform.rs`<br>**(933 行)** | 拆分為 `platform/` 目錄下 **5 個領域子模組**，門面僅 **48 行** | `70eb34a` | ✅ 單檔最大僅 302 行，剪貼簿、終端規格、祖先行程追蹤與系統開啟/Reveal 徹底解耦 |

---

## 2. 目前剩餘大型檔案規模統計 (> 1,000 行)

截至 Phase 14 完成後，**專案中所有核心業務邏輯原始碼已全數低於 1,000 行！**
僅剩單一整合測試檔案超過 1,000 行：

```text
優先級   檔案路徑                              目前行數    主要職責
─────────────────────────────────────────────────────────────────────────────
[P1]    src/file_manager/app/tests.rs       10,835 行   整合測試套件（可按測試領域拆分為多個測試檔案）
```
...
### 推薦第一順位：`src/file_manager/app/tests.rs` (10,835 行)
**目標**：依測試領域拆分為獨立測試檔案。
