...
| **Phase 16** | `src/file_manager/app/tests.rs`<br>**(10,836 行)** | 拆分為 `app/tests/` 目錄下 **20 個領域測試子模組** + `helpers.rs`，門面僅 **65 行** | `8f35378` | ✅ 單檔最大僅 917 行（多數 < 800 行），329 個測試全數遷移、零遺漏零回歸 |
| **Phase 17** | `src/file_manager/tests/pane_test.rs`<br>**(2,362 行)** | 拆分為 `pane/tests/` 目錄下 **5 個領域測試子模組**，門面僅 **24 行** | `bea5e4c` | ✅ 單檔最大僅 774 行，66 個測試全數遷移、移除 path hack，全專案再無千行檔案 |

---

## 2. 目前剩餘大型檔案規模統計 (> 1,000 行)

🎉 **重大里程碑：專案中所有檔案（包含生產核心程式碼與所有測試套件）已 100% 全數低於 1,000 行！**
全專案已無任何超過 1,000 行的檔案。

```text
狀態：全專案 100% < 1,000 行達成！目前所有檔案最大行數為 917 行。
```
...
若需持續精簡生產程式碼（目前生產程式碼單檔皆 < 850 行）：

### 推薦第一順位：`src/file_manager/app/keys/pickers.rs` (815 行)
**目標**：拆解選單快捷鍵交互
- 快捷鍵選擇器按鍵處理
- 搜尋與過濾對話框按鍵
- 主題與配置即時切換按鍵

### 推薦第二順位：`src/file_manager/ui/pane.rs` (756 行)
**目標**：拆解單一面板渲染
- 面板視窗外框與狀態列徽章渲染
- 虛擬化列表視窗與項目滾動範圍計算
- 樹狀/雙欄排版與輔助視覺效果
