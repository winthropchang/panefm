...
- 已把 `f` filter 補到更多清單型面板，現在這幾個都支援一致的搜尋流程：
  - `help`
  - `trash`
  - `task`
  - `bookmark list`
...
- 目前操作規則統一為：
  - 按 `f` 進入搜尋輸入
  - 輸入時即時過濾
  - `Enter` 或第一次 `Esc` 收起輸入框，但保留過濾結果
  - 之後可繼續用 `j/k`、`gg/G`、`J/K`、`Ctrl-d/u` 操作列表

### `goto` 與 regex rename 快捷鍵

- 原本的 `cd <path>` 已改名為：
  - `goto <path>`
- command mode 的路徑補全、Tab 補全、最長共同前綴補全，現在都已跟著改成 `goto ...`。
- 一般模式新增：
  - `R`：直接打開預填好的 `rename-regex ` 命令輸入框
- `rename-regex` 仍維持命令式輸入，因為它本來就需要 pattern / replace 參數。

### `g` leader 面板

- `g` 不再直接把某個功能綁死成單一路徑，而是改成和 `b`、`,`、`m`、`w` 一樣，先打開底部功能面板。
- 目前 `g` 面板已接好：
  - `g`：等同 `gg`，回到列表頂端
  - `t`：打開預填好的 `goto ` 命令輸入框
  - `Esc`：取消
- 這樣做的目的，是讓之後可以繼續擴充其他 `g` 系列快捷鍵，而不用再拆掉既有操作。

### 底部功能面板顯示規則整理

- 所有這類「第一鍵開面板、第二鍵選功能」的底部面板，現在都統一只顯示第二個按鍵，不再重複顯示第一鍵。
- 已整理的面板包含：
  - `g`
  - `,`
  - `m`
...
- 例如：
  ...
  - `w` 面板顯示 `h/j/k/l/c/o`
  - `,` 排序面板只顯示 `m/M/b/B/...`
- 取消項目目前也統一只顯示 `Esc`，避免視覺上再次出現第一鍵前綴。

### 目前測試狀態

- 這一輪完成後，本機測試結果為：
  - `cargo test --quiet` => `276 passed`

### 接下來建議

- `g` 系列之後可以考慮繼續擴充：
  - 例如與 tab / pane / history / 跳轉相關的動作
- 若之後還要新增多段命令，優先沿用目前這種：
  - 第一鍵先開底部面板
  - 第二鍵執行功能
  - 面板只顯示第二鍵
 這樣整體學習成本會低很多，也比較符合目前專案的操作一致性。
